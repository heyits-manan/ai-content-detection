...
export interface DetectionData {
  ...
  real_probability?: number;
  human_probability?: number;
  weighted_ai_probability?: number;
  average_ai_probability?: number;
  average_real_probability?: number;
...
export default function ResultBox({
  ...
  const displayHumanLikeProb =
    detectionData?.average_real_probability ??
    detectionData?.real_probability ??
    detectionData?.average_human_probability ??
    detectionData?.human_probability;
    ...
    <div className="w-full rounded-2xl border border-[var(--color-dark-border)] bg-[var(--color-dark-card)] p-6 space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className={`text-lg font-bold ${isAI ? "text-red-400" : "text-emerald-400"}`}>
              {isAI ? "TRUE" : "FALSE"}
            </p>
          </div>
        </div>
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-4 pt-1">
          ...
          {displayHumanLikeProb !== undefined && (
            <ScoreBar
              label="Overall Human Probability"
              score={displayHumanLikeProb}
              color={displayHumanLikeProb > 0.5 ? "emerald" : "red"}
            />
          )}
        </div>
